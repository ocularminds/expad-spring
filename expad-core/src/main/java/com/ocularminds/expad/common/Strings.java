...
public class Strings {
     ...
     * @param len Size of string to trim
     * @return String (may be null)
     */
    public static String trim(String s, int len) {
        return s != null && s.trim().length() > len ? s.trim().substring(0, len) : s;
    }
    ...
    /**
     * zeropads a long without throwing an Exception (performs modulus
     * operation)
     *
     * @param l the long
    ...
    public static byte[] str2hex(String s, boolean padLeft, byte[] d, int offset) {
        ...
        for (int i = start; i < len + start; i++) {
            d[offset + (i >> 1)]
                    |= Character.digit(s.charAt(i - start), 16) << ((i & 1) == 1 ? 0 : 4);
        }
        return d;
    }
     ...
     *
     * This method will set bits 0 (and 65) if there's a secondary (and
     * tertiary) bitmap (i.e., if the bitmap length is > 64 (and > 128))
     *
     * @param b - the BitSet
    ...
    public static BitSet byte2BitSet(byte[] b, int offset, int maxBits) {
        ...
        int len
                = (maxBits > 128 && b1 && b65) ? 192 : (maxBits > 64 && b1) ? 128 : (maxBits < 64)
                                        ? maxBits
                                        : 64;
     ...
     * Converts a binary representation of a Bitmap field into a Java BitSet.
     *
     * The byte[] will be fully consumed, and fed into the given BitSet starting
     * at bitOffset+1
     *
     * @param bmap - BitSet
     * @param b - hex representation
    ...
    public static BitSet hex2BitSet(byte[] b, int offset, boolean bitZeroMeansExtended) {
        int len
                = bitZeroMeansExtended ? (Character.digit((char) b[offset], 16) & 0x08) == 8
                        ? 128
                        : 64 : 64;
        BitSet bmap = new BitSet(len);
        for (int i = 0; i < len; i++) {
    ...
    public static String formatDouble(double d, int len) {
        ...
        try {
            if (len > 3) {
                prefix = Strings.padleft(prefix, len - 3, ' ');
            }
            suffix = Strings.zeropad(suffix, 2);
        } catch (Exception e) {
            // should not happen
        }
     ...
     * <br>
     * String constraints are: <li>Number must be less than 10 digits</li>
     ...
     * <br>
     * Array constraints are: <li>Number must be less than 10 digits</li>
     * <li>Number must be positive</li>
     *
    ...
    public static String formatAmountConversionRate(double convRate) throws Exception {
        ...
        if (x > 9) {
            bds = Strings.zeropad(bds, bds.length() + x - 9);
        }
        String ret = Strings.zeropadRight(bds, 7);
        return Math.min(9, x) + Strings.takeFirstN(ret, 7);
    }
     ...
     *
     * @see org.jpos.iso.Strings#commaDecode(String)
     ...
     *
     * @see org.jpos.iso.Strings#commaEncode(String[])
